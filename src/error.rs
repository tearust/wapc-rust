...
define_scope! {
	Wapc {
		...
		HostCallFailure as v => HostCallFailure, format!("Error occurred during host call: {}", v.0), @Debug, single(&v.0);
		GuestCallFailure as v => GuestCallFailure, format!("Guest call failure: {}", v.0), @Debug, single(&v.0);
	}
}
